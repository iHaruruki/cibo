...
class FrontCameraNode(Node):
    ...
    def setup_opencv_window(self):
        """Setup OpenCV window for ROI selection"""
        try:
            cv2.namedWindow('Front Camera ROI', cv2.WINDOW_AUTOSIZE)
            cv2.setMouseCallback('Front Camera ROI', self.mouse_callback)
    ...
    def process_face_mesh_landmarks(self, face_landmarks_list, roi_offset_x=0, roi_offset_y=0, image_width=1, image_height=1):
        """Process detailed face mesh landmarks (468 points)"""
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # Process with MediaPipe Face Mesh for detailed face analysis
            ...
            cv2.imshow('Front Camera ROI', display_image)
            ...
            pose_landmarks = self.process_landmarks(
                holistic_results.pose_landmarks, roi_x, roi_y, roi_width, roi_height
            )
            
            # Use detailed face mesh landmarks (468 points)
            face_landmarks = self.process_face_mesh_landmarks(
                face_mesh_results.multi_face_landmarks, roi_x, roi_y, roi_width, roi_height
            )
            
            left_hand_landmarks = self.process_landmarks(
                holistic_results.left_hand_landmarks, roi_x, roi_y, roi_width, roi_height
            )
            
            right_hand_landmarks = self.process_landmarks(
                holistic_results.right_hand_landmarks, roi_x, roi_y, roi_width, roi_height
            )
